...
mytheme <- theme_classic() +
  theme(axis.text = element_text(color = "black"), 
        legend.position = "top")
hppal31 <- hp(n=31, house = "Hufflepuff")
...
```{r, fig.height = 3, fig.width = 4, echo = FALSE, fig.cap="\\label{fig:figs} Total Annual Coal Production in Harlan and Dickenson Counties"}
ton <- ggplot(minedata.clean, aes(x = year, y = total.stons, color =
                                              countystr)) +
  geom_line() +
  labs(x = "Year", y = "Annual Production (short tons)", color = "County") +
  xlim(2000, 2011) +
...
employee <- ggplot(minedata.clean, aes(x = year, y = total.employee, color = countystr)) +
  ...
  xlim(2001, 2011) +
  scale_color_manual(values=hppal2) +
  scale_x_continuous("Year", c(2001, 2002, 2003, 2004, 2005, 2006, 2007, 2008, 2009, 2010, 2011))
print(employee)
```


```{r, include = FALSE}
#harlan.plot <- ggplot(harlan.clean, aes(x = year, y = average.employees, color = mine.name)) +
 # geom_line() +
